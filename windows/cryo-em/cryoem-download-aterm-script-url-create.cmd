...
REM service name
set "SERVICE=unimelb.asset.download.aterm.script.url.create"
...
set "COMMAND=%COMMAND% :download ^< %NAMESPACES% :token ^< :role -type role %ROLE% :to now+%EXPIRE_DAYS%day ^> :ncsr %NCSR% :verbose %VERBOSE% :overwrite %OVERWRITE% ^> :token ^< :perm ^< :resource -type role:namespace %PROJECT%: :access ADMINISTER ^> ^>"
...
echo     --ncsr ^<ncsr^>                     number of concurrent service requests. Defaults to %NCSR%
